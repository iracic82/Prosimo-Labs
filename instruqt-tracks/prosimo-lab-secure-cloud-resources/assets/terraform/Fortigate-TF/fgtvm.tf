...
//
data "aws_network_interface" "gwlbip" {
  depends_on = [aws_lb.gateway_lb]
  filter {
    name   = "vpc-id"
  ...
  filter {
    name   = "description"
    values = ["ELB gwy/${aws_lb.gateway_lb.name}/*"] #["*ELB*"]
  }
  filter {
...
data "aws_network_interface" "gwlbipaz2" {
  depends_on = [aws_lb.gateway_lb]
  filter {
    name   = "vpc-id"
    values = ["${aws_vpc.fgtvm-vpc.id}"]
...
resource "aws_instance" "fgtvm" {
  ...
  user_data = templatefile("${var.bootstrap-fgtvm}", {
    ...
    endpointip   = "${data.aws_network_interface.gwlbip.private_ip}"
    endpointip2  = "${data.aws_network_interface.gwlbipaz2.private_ip}"
  })
